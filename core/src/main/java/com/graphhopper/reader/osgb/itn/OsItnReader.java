 ...
 */
/**
 * This class parses an OS ITN xml file and creates a graph from it. It does so in a two phase
...
public class OsItnReader extends AbstractOsReader<Long>
{
	...
	/**
	 * This method processes the list of NodeIds and checks if any nodes have a NoEntry Tag. If it
	 * does then it adds a shadow node and an extra way as a OneWay. Once it has run out of NoEntry
	 * nodes to process it passes the remainder on to processBarriers to check for barriers and
	 * construct the remaining way
	 *
	 * @param way
	 * @param wayNodes OSITNNode objects for the way nodes, ie not including the start and end nodes
	 * @param osmNodeIds Node Ids of all Nodes, including the start and end nodes
	 * @param wayFlags
	 * @param wayOsmId
	 * @return
	 */
	...
	/**
	 * This method takes the supplied list of osmNodeIds and checks for barriers. If it find them it
	 * adds a shadow node with zero length at the same location and adds ways such that they are
	 * connected. The remaining way once it has processed all barriers is just created as a series
	 * of ways between the remaining nodes
	 *
	 * @param way Not really used much. Could be refactored so this parameter is not required.
	 * @param osmNodeIds Ids of the nodes to check for barriers
	 * @param wayFlags
	 * @param wayOsmId
	 * @return
	 */
	...
	/**
	 * FROM OSMReader: Stores only osmWayIds which are required for relations This copy stores all
	 * mappings because getOsmIdStoreRequiredSet() isn't populated until processStage3 and this call
	 * is made in processStage2 so we would not have any entries
	 */
	...
	private OSITNNode addBarrierNode( long nodeId, boolean forceAsTower )
	{
		...
		} else
		{
			...
			{
				logger.error("ArrayIndexOutOfBoundsException occured", e);
			}
		}
